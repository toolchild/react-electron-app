...
export default class CountryCodeField extends React.Component {
  ...
  constructor(props) {
    ...
    this.remCountryCode = this.remCountryCode.bind(this);
    this.onUpdateInput = this.onUpdateInput.bind(this);
    this.renderSelectedCountryCodes = this.renderSelectedCountryCodes.bind(this);
    ...
    this.dataSourceConfig = {text: 'code', value: 'code',}
  }
    ...
    if (!_.includes(this.state.selectedCountries, this.state.countryCodeSearchText) && this.state.countryCodeSearchText.length ===2 ) {
      this.setState((prevState, props) => {
        return {
          ...prevState,
          countryCodeSearchText: '',
          selectedCountries: _.concat(this.state.countryCodeSearchText, prevState.selectedCountries)
        }
      });
    } else if (this.state.countryCodeSearchText !== '') {
      this.setState((prevState, props) => {
        return {
          ...prevState,
  ...
  renderSelectedCountryCodes() {
    ...
    for (let i = 0; i < this.state.selectedCountries.length; i++) {
      selectedCountryCodeButtons = _.concat(selectedCountryCodeButtons,// does this mutate?
        <RaisedButton key={i} label={this.state.selectedCountries[i]} className="raised-button"
                      onClick={this.remCountryCode}/>);
    }
    return selectedCountryCodeButtons;
  }
}